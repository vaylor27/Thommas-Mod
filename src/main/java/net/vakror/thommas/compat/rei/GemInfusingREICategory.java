...
public class GemInfusingREICategory implements DisplayCategory<GemInfusingREIDisplay> {
	private final EntryStack<ItemStack> block = EntryStacks.of(new ItemStack(ModBlocks.GEM_INFUSING_STATION));

	@Override
	public @NotNull CategoryIdentifier<GemInfusingREIDisplay> getCategoryIdentifier() {
	...
	public Text getTitle() {
		return Text.literal("Gem Infusing");
	}

	@SuppressWarnings("UnstableApiUsage")
	@Override
	public @NotNull List<Widget> setupDisplay(GemInfusingREIDisplay display, Rectangle bounds) {
		...
		Slot inputSlot = Widgets.createSlot(new Point(center.x,center.y)).entry(inputs.get(0).get(0)).markInput();
		widgets.add(inputSlot);
		widgets.add(Widgets.createSlot(new Point(center.x,center.y + 20)).entry(output));

		widgets.add(Widgets.createDrawableWidget(((helper, matrices, mouseX, mouseY, delta) -> {
			renderer.drawFluid(matrices, new FluidStack(FluidVariant.of(ModFluids.HONEY_STILL), FluidStack.convertDropletsToMb(FluidConstants.BUCKET * 50)), center.x - 30, center.y - 25, 16, 61,
	...
	private static long getEnergyAmount(Item item) {
		return EnergyAmountsUtil.InfusingEnergyAmounts.getEnergyAmountFromItem(item);
	}
	...
	@Override
	public int getDisplayWidth(GemInfusingREIDisplay display) {
		return 250;
	}
}
