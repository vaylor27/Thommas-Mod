...
public class MassiveCrystalChestEntity extends GenericChestEntity {

    private final DefaultedList<ItemStack> topStacks;
    private boolean inventoryTouched;

    public MassiveCrystalChestEntity(BlockPos pos, BlockState state) {
        super(ChestTypes.MASSIVE_CRYSTAL, pos, state);
    ...
    public void tick() {
        if (world != null && !world.isClient && this.inventoryTouched) {
            ...
            viewers.forEach(player -> ServerPlayNetworking.send(player, Thommas.UPDATE_INV_PACKET_ID_MASSIVE, buf));
