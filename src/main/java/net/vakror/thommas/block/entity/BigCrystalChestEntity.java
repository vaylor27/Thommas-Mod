...
public class BigCrystalChestEntity extends GenericChestEntity {

    private final DefaultedList<ItemStack> topStacks;
    private boolean inventoryTouched;

    public BigCrystalChestEntity(BlockPos pos, BlockState state) {
        super(ChestTypes.BIG_CRYSTAL, pos, state);
    ...
    public void tick() {
        if (world != null && !world.isClient && this.inventoryTouched) {
            ...
            viewers.forEach(player -> ServerPlayNetworking.send(player, Thommas.UPDATE_INV_PACKET_ID_BIG, buf));
