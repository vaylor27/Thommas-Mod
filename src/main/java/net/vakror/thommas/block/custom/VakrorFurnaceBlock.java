...
public class VakrorFurnaceBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(world, type, ModBlockEntities.VAKROR_FURNACE);
    }

    @Nullable
    @Override
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new VakrorFurnaceEntity(pos, state);
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (be instanceof VakrorFurnaceEntity furnace) {
                if (world instanceof ServerWorld) {
                    ItemScatterer.spawn(world, pos, furnace);
                    ((VakrorFurnaceEntity)be).getRecipesUsedAndDropExperience((ServerWorld)world, Vec3d.ofCenter(pos));
                }
    ...
    private void openContainer(World world, BlockPos blockPos, PlayerEntity playerEntity) {
        ...
        if (blockEntity instanceof VakrorFurnaceEntity) {
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            ...
            if (blockEntity instanceof VakrorFurnaceEntity) {
                ((VakrorFurnaceEntity) blockEntity).setCustomName(itemStack.getName());
            }
        }
    }
    ...
    @Nullable
    protected static <T extends BlockEntity> BlockEntityTicker<T> checkType(World world, BlockEntityType<T> givenType, BlockEntityType<? extends VakrorFurnaceEntity> expectedType) {
        return world.isClient ? null : checkType(givenType, expectedType, VakrorFurnaceEntity::tick);
    }
