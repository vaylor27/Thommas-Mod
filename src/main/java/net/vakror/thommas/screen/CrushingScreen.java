...
public class CrushingScreen extends HandledScreen<CrushingScreenHandler> {
    private static final Identifier TEXTURE =
            new Identifier(Thommas.MOD_ID, "textures/gui/crusher_gui.png");
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (this.handler.blockEntity.inventory.get(2).getItem().equals(ModItems.CRUSHING_HEAD)) {
            renderCrushingHead(matrices, x, y);
        }
    ...
    private void renderCrushingHead(MatrixStack matrices, int x, int y) {
            drawTexture(matrices, x + 100, y + 22, 178, 12, 25, 16);
    }

    private void renderProgressArrow(MatrixStack matrices, int x, int y) {
        if(handler.isCrafting()) {
            drawTexture(matrices, x + 96, y + 43, 176, 2, handler.getScaledProgress(), 7);
        }
        energyInfoArea.draw(matrices);
    }
